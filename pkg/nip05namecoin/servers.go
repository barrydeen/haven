...
// Transport selects how an ElectrumX server is dialed. HAVEN ships
// TCP and TCP+TLS only — WebSocket transports are intentionally
// excluded since HAVEN runs as a server-side process where raw TCP
// is always available.
type Transport int
...
type ElectrumxServer struct {
	...
	// When Transport is the zero value, UseSSL selects between
	// TransportTCPTLS (UseSSL=true) and TransportTCP (UseSSL=false).
	UseSSL bool
	...
	// Transport selects the dial transport. Zero value falls back to
	// UseSSL: if UseSSL is true it's TCP+TLS, otherwise plain TCP.
	Transport Transport
}
...
// ElectrumX servers, ordered by operational history. Operators can
// patch this list at compile time if they want to point HAVEN at a
// local Namecoin daemon's ElectrumX bridge or a private mirror.
