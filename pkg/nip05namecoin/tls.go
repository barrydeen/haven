...
// standard library still succeed when the operator publishes an updated
// cert with a known fingerprint.
...
// to the given server. When UsePinnedTrustStore is false, the returned
// config is the stdlib default (system roots only). When true, the config
// trusts either the system roots *or* one of the pinned self-signed certs.
func tlsConfigFor(server ElectrumxServer) *tls.Config {
	...
	cfg := &tls.Config{
		...
		VerifyPeerCertificate: func(rawCerts [][]byte, _ [][]*x509.Certificate) error {
			if len(rawCerts) == 0 {
				return errors.New("nip05namecoin: no peer certificates presented")
			}
			certs := make([]*x509.Certificate, 0, len(rawCerts))
			for _, raw := range rawCerts {
				c, err := x509.ParseCertificate(raw)
				if err != nil {
					return fmt.Errorf("nip05namecoin: parse peer cert: %w", err)
				}
			...
			//    cert across multiple hostnames.
			if _, err := leaf.Verify(x509.VerifyOptions{
				Roots:         pinnedPool,
				Intermediates: intermediates,
			}); err == nil {
			...
			return fmt.Errorf("nip05namecoin: peer cert for %s is not in system or pinned trust store", server.Host)
		},
	}
